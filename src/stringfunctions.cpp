...
std::vector<std::string> Rex(const std::string& s, std::string r)
{
  ...
  for(auto i = R; i != std::sregex_iterator(); ++i) res.push_back(i->str());
  return res;
}
...
{
  std::vector<std::string> res;
  for(auto i : s) for(auto j : Rex(i, r)) res.push_back(j);
  return res;
}
...
bool RexIn(const std::string& s, std::string r){ return Rex(s, r).size() > 0;}
...
stringloc(const std::string& s, const std::string& r, std::string sf)
{
  ...
  for (std::sregex_iterator i = wb; i != we; ++i)
    {
      std::smatch match = *i;
      int match_int = match.position();
      if(sf != "start") match_int += match.length();
...
std::vector<std::string> splitter(const std::string& s, const std::string& m)
{
  ...
  if(sl[0].size() == 0) {res.push_back(s); return res;}
  int n = sl[0].size();
  RC.push_back(s.substr(0, sl[0][0]));
  if(n > 1) for(int i = 1; i < n; i++)
  {
    RC.push_back(s.substr(sl[1][i - 1], sl[0][i] - sl[1][i - 1]));
  }
  RC.push_back(s.substr(sl[1][n - 1], s.length() - sl[1][n - 1]));
  for(auto i : RC) if(i.length() != 0) res.push_back(i);
  return res;
}
...
{
  int firstpos  = 0;
  int secondpos = subject.length();
  std::vector<int> FPV = stringloc(subject, precarve, "end");
  std::vector<int> SPV = stringloc(subject, postcarve, "start");
  int fpvs = FPV.size();
  int spvs = SPV.size();
  if((fpvs == 0) && (spvs > 0)) secondpos = SPV[spvs - 1];
  if((fpvs > 0) && (spvs == 0)) firstpos = FPV[0];
  if((fpvs > 0) && (spvs > 0))
  {
    firstpos = FPV[0];
    for(auto i : SPV) if(i > firstpos){ secondpos = i; break;}
  }
...
std::vector<uint16_t> strtoint(std::string x)
{
  ...
  for(auto i : x) {uint16_t a = i; res.push_back(a % 255);}
  return res;
}
...
std::vector<float> getnums(const std::string& s)
{
  ...
  std::string numstring = "(-)?(\\.)?\\d+(\\.)?\\d*";
  std::vector<std::string> strs = Rex(s, numstring);
  for(auto i : strs) res.push_back(stof(i));
  return res;
}
...
std::vector<int> getints(const std::string& s)
{
  ...
  std::vector<std::string> strs  = Rex(s, numstring);
  for(auto i : strs) res.push_back(stoi(i));
  return res;
}
...
std::vector<unsigned char> bytesFromArray(const std::string& s)
  {
    if(s.size() == 0) Rcpp::stop("Zero-length string passed to bytesFromArray");
    std::vector<int> tmpvec, res;
    std::vector<unsigned char> resvec;
    for(auto a : s)
    {
      if(a > 47 && a < 58)  tmpvec.push_back(a - 48);
      if(a > 64 && a < 71)  tmpvec.push_back(a - 55);
      if(a > 96 && a < 103) tmpvec.push_back(a - 87);
    }
    unsigned ts = tmpvec.size();
    if(ts == 0) Rcpp::stop("arrayFromBytes not given a byte string");
    for(unsigned i = 0; i < ts; i++) if(i % 2 == 0) tmpvec[i] = 16 * tmpvec[i];
    for(unsigned i = 0; i < (ts - 1); i++)
    {
      if(i % 2 == 0) res.push_back(tmpvec[i] + tmpvec[i+1]);
    }
    if((ts - 1) % 2 == 0) res.push_back(tmpvec[ts - 1]);
    for(auto i : res) resvec.push_back(i);
    return resvec;
  }
...
std::vector<float> matmul(std::vector<float> b, std::vector<float> a)
{
  ...
  if(a.size() != b.size()) Rcpp::stop("Vectors must have same size.");
  if(a.size() != 9) Rcpp::stop("Vectors must have size 9.");

  newmat.push_back(a[0] * b[0] + a[3] * b[1] + a[6] * b[2]);
  newmat.push_back(a[1] * b[0] + a[4] * b[1] + a[7] * b[2]);
  newmat.push_back(a[2] * b[0] + a[5] * b[1] + a[8] * b[2]);

  newmat.push_back(a[0] * b[3] + a[3] * b[4] + a[6] * b[5]);
  newmat.push_back(a[1] * b[3] + a[4] * b[4] + a[7] * b[5]);
  newmat.push_back(a[2] * b[3] + a[5] * b[4] + a[8] * b[5]);

  newmat.push_back(a[0] * b[6] + a[3] * b[7] + a[6] * b[8]);
  newmat.push_back(a[1] * b[6] + a[4] * b[7] + a[7] * b[8]);
  newmat.push_back(a[2] * b[6] + a[5] * b[7] + a[8] * b[8]);
...
{
  std::vector<float> r;
  for(auto i : b) r.push_back(std::stof(i));
  return r;
}
...
std::string intToHexstring(int i)
{
  ...
  std::string res;
  int firstnum = i / (16 * 16 * 16); i -= firstnum * (16 * 16 * 16);
  int secondnum = i / (16 * 16); i -= secondnum * (16 * 16);
  int thirdnum = i / 16; i -= thirdnum * 16;
  res += hex[firstnum];
  res += hex[secondnum];
  res += hex[thirdnum];
  res += hex[i];
...
std::vector<std::string> splitfours(std::string s)
{
  ...
  for(unsigned i = 0; i < s.length()/4; i++) res.push_back(s.substr(i * 4, 4));
  return res;
}
...
std::vector<std::string> splittwos(std::string s)
{
  std::vector<std::string> res;
  if(s.size() == 0) return res;
  while(s.size() % 2 != 0) s += '0';
  for(unsigned i = 0; i < s.length()/2; i++) res.push_back(s.substr(i * 2, 2));
  return res;
}
...
std::string byteStringToString(const std::string& s)
{
  ...
  std::string res;
  for(auto i : sv) uv.push_back((unsigned) stoul("0x" + i, nullptr, 0));
  for(auto i : uv) { if(i > 255) i = 255; res += (char) i;}
  return res;
}
...
{
  std::vector<int> res;
  for (auto i : Rex(ds, "\\d+ 0 R")) res.push_back(stoi(splitter(i, " ")[0]));
  return res;
}
...
bool isDictString(const std::string& s) {return stringloc(s, "<<").size() > 0;}

/*---------------------------------------------------------------------------*/

char symbol_type(const char c)
{
  if(c > 0x40 && c < 0x5b) return 'L';
  if(c > 0x60 && c < 0x7b) return 'L' ;
  if(c > 0x2f && c < 0x3a) return 'D';
  if(c == ' ' || c == 0x0d || c == 0x0a ) return ' ';
  return c;
}
